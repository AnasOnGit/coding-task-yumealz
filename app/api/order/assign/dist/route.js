"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.PUT = void 0;
var client_1 = require("@prisma/client");
/**
 * This endpoint is for assigning orders to a captain.
 * endpoint usage:/api/order/assign/
 */
function PUT(request) {
    return __awaiter(this, void 0, void 0, function () {
        var prisma, data, order_ids, captain_id_1, queryResult, err_1;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    prisma = new client_1.PrismaClient();
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, 5, 7]);
                    return [4 /*yield*/, request.json()];
                case 2:
                    data = _a.sent();
                    order_ids = data.ordersId;
                    captain_id_1 = data.captainId;
                    return [4 /*yield*/, Promise.all(order_ids.map(function (order_id) { return __awaiter(_this, void 0, void 0, function () {
                            var order, queryResult, ordersAccepted;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0: return [4 /*yield*/, prisma.order.findUnique({
                                            where: {
                                                id: Number(order_id)
                                            }
                                        })];
                                    case 1:
                                        order = _a.sent();
                                        if (order === null || order === void 0 ? void 0 : order.captain_id) {
                                            return [2 /*return*/, new Response(JSON.stringify({
                                                    message: "Captain is already assigned!",
                                                    captain_assigned_id: order.captain_id
                                                }), {
                                                    status: 400
                                                })];
                                        }
                                        return [4 /*yield*/, prisma.order.update({
                                                where: {
                                                    id: Number(order_id)
                                                },
                                                data: {
                                                    captain_id: Number(captain_id_1)
                                                }
                                            })];
                                    case 2:
                                        queryResult = _a.sent();
                                        return [4 /*yield*/, prisma.captainStatistic.updateMany({
                                                where: {
                                                    captain_id: Number(captain_id_1)
                                                },
                                                data: {
                                                    total_orders_accepted: {
                                                        increment: 1
                                                    }
                                                }
                                            })];
                                    case 3:
                                        ordersAccepted = _a.sent();
                                        return [2 /*return*/, queryResult];
                                }
                            });
                        }); }))];
                case 3:
                    queryResult = _a.sent();
                    return [2 /*return*/, new Response(JSON.stringify({
                            message: "Captain is assigned successfully",
                            data: queryResult,
                            success: true
                        }))];
                case 4:
                    err_1 = _a.sent();
                    console.log(err_1);
                    return [2 /*return*/, new Response(JSON.stringify({ message: err_1 }), {
                            status: 500
                        })];
                case 5: return [4 /*yield*/, prisma.$disconnect()];
                case 6:
                    _a.sent();
                    return [7 /*endfinally*/];
                case 7: return [2 /*return*/];
            }
        });
    });
}
exports.PUT = PUT;
